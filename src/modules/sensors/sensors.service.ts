...
export class SensorsService {
  constructor(
      ...
      async getData(query: QueryParamsDto): Promise<Sensor[]> {
          ...
          if (keyword) {
              ...
              queryBuilder.andWhere(
                  new Brackets(qb => {
                      if (isDateValid) {
                    qb.orWhere('sensorsdata.time = :exactTime', { exactTime: `${decodedKeyword}` });
                }
                      if (isNumberValid) {
      ...
      async countLightGreaterThan800(): Promise<number> {
          const count = await this.sensorsDataRepository.createQueryBuilder('sensorsdata')
      ...
      convertUtcToCustomFormat(utcDate: string): string {
          ...
          const milliseconds = String(date.getUTCMilliseconds()).padStart(3, '0').padEnd(6, '0'); 
